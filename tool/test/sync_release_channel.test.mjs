...
test('ignores a published release whose tag is not reachable from main', () => {
  const manifest = selectReleaseChannel(
    [
    ...
    {
      sourceCommits: new Map([['v1.1.3', 'a'.repeat(40)]]),
    },
  );

  assert.equal(manifest.preview.tag, 'v1.1.3');
  assert.equal(manifest.preview.sourceCommit, 'a'.repeat(40));
});
