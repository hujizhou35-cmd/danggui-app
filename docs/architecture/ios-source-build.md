...
`danggui-ios-source-v1.1.4.zip` 是可审计、可复现解包的 Flutter/iOS 源码交付，不是 IPA，也不能直接安装到普通 iPhone。为了让接收者能运行与标签 CI 相同的完整测试和平台审计，压缩包保留全部已跟踪的 Flutter 项目源码（包括 Android 平台源码、`test/` 和 `integration_test/`），但不包含签名证书、描述文件、密钥、构建缓存、`dist` 或用户数据。既有 v1.1.3、v1.1.2、v1.1.0 与 v1.0.0 Release 及其历史附件继续保留。
...
```bash
grep '  ./danggui-ios-source-v1.1.4.zip$' SHA256SUMS | shasum -a 256 -c -
unzip danggui-ios-source-v1.1.4.zip
cd danggui-ios-source-v1.1.4
...
`build_ios_unsigned.sh` 会从 `pubspec.yaml` 读取 `1.1.4+5`，生成 `dist/ios/danggui-ios-unsigned.app.zip` 并复查 Bundle ID、最低系统版本、竖屏限制、后台模式和远程通知 entitlement。平台静态审计另会核对 `NSAlarmKitUsageDescription`、AlarmKit 的 iOS 26 可用性保护，以及唯一允许的 `com.apple.developer.usernotifications.time-sensitive` entitlement。该 `.app.zip` 只用于证明源码可构建，仍不是 IPA。
