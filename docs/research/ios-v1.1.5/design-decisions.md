...
- **决定**：README 真机截图、主 Android 制品构建和 API 24/36 设备验收都在进入 Gradle/模拟器前显式安装 `cmake;3.22.1`；离线审计按 job 分段锁定这一合同，并用主动破坏测试证明缺失任一入口都会失败。
...
## DD33 — Android 编辑器验收命中目标字段子树后只短按一次

- **决定**：设备验收先确认目标 `EditableText` 尚未聚焦，再通过 `EditableTextState.renderEditable` 取得经完整 transform 映射后的可见文本矩形，并在其中无副作用扫描 5×5 候选点。目标 `TextField` 下所有已挂载 `RenderObjectElement` 的真实 render object 组成身份集合；候选只有在当前 Flutter view 的 hit-test 路径与该集合存在精确对象交集时才可使用。选择正文上方的首个有效点后只发送一次 16ms pointer down/up，随后必须由同一 `FocusNode`、真实 IME、三层 inset/几何合同和最终持久化共同证明成功。失败证据只记录坐标、矩形和命中目标类型，不记录用户文本。
