...
本文档是 v1.1.0 稳定版门禁的证据清单。模拟器作业包含两个不能混称的阶段：第一阶段使用 Debug 测试宿主和临时 VM-service 清单完成真实键盘、数据保留、权限、提醒与通知交互验收；第二阶段卸载测试包，下载并原样安装同一 workflow SHA 的 `android-linux` 通用 Release-mode APK，只用主机 ADB/UIAutomator 验证该精确二进制的冷启动、包名/版本、基础导航和无崩溃。两者都不证明用户提供的正式 v1.0.0 APK 能在特定 OEM 实体机无损升级到 v1.1.0，也不代替锁屏、声音或振动体验验收。

## API 24 / 36 模拟器自动证据

历史基线 v1.0.0 的可追溯证据来自标签提交 `b6dc50594abdb769080a738dd64550d26bc64d36` 和 [GitHub Actions run 32609272408](https://github.com/hujizhou35-cmd/danggui-app/actions/runs/32609272408)。该工作流的 Android 正式构建、API 24、API 36 与 unsigned iOS 四个作业全部成功。公开交付页面是 [v1.0.0 Pre-release](https://github.com/hujizhou35-cmd/danggui-app/releases/tag/v1.0.0)；v1.1.0 必须产生自己的标签证据，并在下方以该正式 v1.0.0 包为升级起点。
...
- `release-binary-apk.sha256`、`release-binary-apk-metadata.json`、`release-binary-apk-verification.txt`：记录 workflow SHA 和下载文件 SHA-256，并再次验证 `com.danggui.memo`、`1.1.0`、versionCode `2`、min/target SDK `24/36`、`debuggable=false`、权限、签名及 manifest 合同。
...
标签 run 的通过合同要求 API 24 与 API 36 交互阶段各完成 37/37 项断言，覆盖事项、提醒、笔记、文件夹、过往、设置六个数据域，以及 SQLite `quick_check`/外键完整性、事项卡提醒文案、AlarmManager 和真实系统通知；API 36 还必须观察真实系统权限弹窗、通过系统 UI 点击授权，并验证最终 `POST_NOTIFICATIONS` 授权状态。v1.1.0 标签 Artifact 尚未产生前不得把这些要求写成已通过；即使通过，也不得解释为旧 schema 迁移或实体机 OEM 行为已经验收。
...
### v1.0.0 → v1.1.0 正式包覆盖安装与数据

- [ ] 安装用户提供且指纹已核对的正式 v1.0.0 APK，创建至少一个含日期、计划、正文和提醒的事项，并创建笔记与过往正文
- [ ] 记录覆盖前事项与提醒截图：__________
- [ ] 使用同一应用 ID、同一正式签名的 v1.1.0 APK 执行覆盖安装，未卸载、未清除数据
- [ ] 覆盖安装后事项标题、日期、计划、正文、提醒时间和开关，以及笔记、过往和设置均保留
- [ ] 事项、笔记与过往进入后都能输入、保存、返回并重进；键盘收起后顶栏和工具栏不消失
- [ ] 新建事项默认当天且可清除日期；提醒状态、权限受限与系统设置入口可用
- [ ] 冷启动完整显示水彩球与嫩芽、当归标题、隐私文案和加载状态至少一秒
