...
> [!IMPORTANT]
> 当归 [v1.1.0](https://github.com/hujizhou35-cmd/danggui-app/releases/tag/v1.1.0) は **Pre-release（公開プレリリース）**として公開されています。Android では正式署名済みインストールパッケージを提供します。iOS は決定的なソース ZIP と未署名ビルドの証跡のみで、IPA/TestFlight は提供しません。実機でのリリース受け入れ試験は未完了のため、安定版ではありません。
...
- v1.1.0 ではタスク、ノート、「過去」を常に編集可能にし、新規タスクの日付を今日に設定し、通知状態を管理できるようにしました。完全なブランド起動画面を 1 秒以上表示します。
...
実機での OEM ごとの挙動、ロック画面通知、音、バイブレーション、スヌーズ、および正式版 v1.0.0 からの上書き更新の最終署名は未完了です。そのため v1.1.0 は Pre-release のままであり、安定版ではありません。
